...
export const getAllCompanyUsers = action(IdForCompanySchemaString, async ({ companyId }) => {
  const endpoint = `${apiUrl}${SAGE_ONE_USER_COMPANY.GET.GET_ALL_COMPANY_USERS}?companyId=${companyId}`;

  try {
    const response = await fetch(endpoint, { next: { tags: ["all-company-users"] } });
    if (!response.ok) {
...
export const getSpecificCompanyUser = action(IdSchemaString, async ({ id }) => {
  ...
  try {
    const response = await fetch(endpoint, { next: { tags: ["company-user"] } });
    if (!response.ok) {
      throw new ActionError(`Error: ${response.status}`);
    }
...
export const createCompanyUser = action(CreateCompanyUserRequestModelSchema, async (userCredentials) => {
...
export const updateCompanyUser = action(UpdateCompanyUserRequestModelSchema, async (user) => {
...
export const loginCompanyUser = action(LoginCompanyUserSchema, async ({ rememberMe, ...loginCredentials }) => {
...
export const updateCompanyUserPassword = action(UpdateCompanyUserPasswordSchema, async ({ passwordConfirmation, email, ...rest }) => {
  // Convert password to base64
  rest.newPassword = toBase64(rest.newPassword);

  const endpoint = `${apiUrl}${SAGE_ONE_USER_COMPANY.PUT.UPDATE_COMPANY_USER_PASSWORD}?email=${encodeURIComponent(email)}&newPassword=${
    rest.newPassword
  }`;
...
export const resetCompanyUserAccountPassword = action(EmailOnlySchema, async ({ email }) => {
  const endpoint = `${apiUrl}${SAGE_ONE_USER_COMPANY.POST.RESET_COMPANY_USER_PASSWORD}?email=${encodeURIComponent(email)}`;
...
export const verifyCompanyUserOTP = action(OTPCompanyUserSchema, async ({ email, code }) => {
  const endpoint = `${apiUrl}${SAGE_ONE_USER_COMPANY.PUT.VERIFY_COMPANY_USER_OTP}?email=${encodeURIComponent(email)}&code=${code}`;
