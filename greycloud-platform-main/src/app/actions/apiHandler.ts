...
var token=session.token;
  ...
  if (API_LOGGING) {
    const masked = token ? `Bearer ${String(token).slice(0, 8)}...` : "(no-token)";
    console.log(`[apiPut] PUT ${url} - headers:`, { ...headers, Authorization: masked }, "payload:", payload);
  }
  ...
  try {
    ...
    if (API_LOGGING) console.log(`[apiPut] ${url} -> ${response.status} ${response.statusText}`);
    ...
    if (!response.ok) {
      ...
      if (API_LOGGING) {
        console.error(`[apiPut] ERROR ${url} - status: ${response.status} - body:`, bodyText);
      }
    ...
    if (API_LOGGING) console.error(`[apiPut] Network/Fetch error for ${url}:`, err);
    throw err;
  }
}
