...
export const getAllAssetGroups = action(EmptySchema, async () => {
  ...
  try {
    const response = await serverFetch(endpoint, { next: { tags: ["asset-group"] } as any });
    if (!response.ok) {
...
export const getSpecificAssetDepreciationHistory = action(GetSpecificAssetDepreciationHistorySchema, async ({ assetid }) => {
  ...
  try {
    const response = await serverFetch(endpoint, { next: { tags: ["asset-depreciation-history"] } as any });
...
export const getFutureAssetDepreciationHistory = action(GetSpecificAssetDepreciationHistorySchema, async ({ assetid }) => {
  ...
  try {
    const response = await serverFetch(endpoint, { next: { tags: ["asset-depreciation-future"] } as any });
